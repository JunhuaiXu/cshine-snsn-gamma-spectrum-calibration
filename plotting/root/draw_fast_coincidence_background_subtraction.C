...
TH1* require_histogram_fast(TFile& file, const char* name) {
  TH1* histogram = 0;
...
TH1D* copy_and_rebin_fast(const TH1& source, const char* name) {
...
void style_axis_fast(TH1& histogram) {
...
void draw_fast_coincidence_background_subtraction(
    const char* analysis_root,
    const char* output_directory,
    bool force = false) {
  const TString base_directory = TString::Format(
      "%s/DataPreprocessing/step10-DifferentTriggerMode/RemoveSSDM2",
      analysis_root);
  const TString signal_path =
      base_directory + "/step4-convert.0308.data/all_recon.root";
  const TString random_path =
      base_directory + "/step4-convert.0308.bkg/all_recon.root";
  const TString reference_path =
      base_directory + "/step4-convert.0308.data/spectrum_110.root";
  const TString output_subdirectory =
      TString(output_directory) + "/fast_coincidence_background_subtraction";
  gSystem->mkdir(output_subdirectory, true);
  const TString output_pdf = output_subdirectory +
      "/cshine_gamma_fast_coincidence_background_subtraction_root_reference.pdf";
  const TString output_png = output_subdirectory +
      "/cshine_gamma_fast_coincidence_background_subtraction_root_reference.png";
  ...
  TFile random_file(random_path, "READ");
  TFile reference_file(reference_path, "READ");
  if (signal_file.IsZombie() || random_file.IsZombie() ||
      reference_file.IsZombie()) {
    throw std::runtime_error("cannot open one or more fast-coincidence ROOT inputs");
  }

  TH1D* signal = copy_and_rebin_fast(
      *require_histogram_fast(signal_file, "h_total_E_M1"), "fast_coincidence");
  TH1D* random_background = copy_and_rebin_fast(
      *require_histogram_fast(random_file, "h_total_E_M1"), "random_window");
  TH1* reference = require_histogram_fast(reference_file, "histDiff");

  TH1D* difference =
      static_cast<TH1D*>(signal->Clone("histDiff_candidate"));
  ...
  for (int bin = 0; bin <= difference->GetNbinsX() + 1; ++bin) {
    ...
    maximum_error_difference =
        std::max(maximum_error_difference, error_difference);
  ...
  gStyle->SetOptStat(0);
  TCanvas canvas("fast_coincidence", "fast coincidence", 1500, 610);
  ...
  style_axis_fast(*signal);
  signal->GetXaxis()->SetRangeUser(0.0, 200.0);
  signal->GetYaxis()->SetRangeUser(5e-1, 1e8);
  signal->SetLineColor(kBlack);
  signal->SetLineWidth(2);
  random_background->SetLineColor(kRed + 1);
  random_background->SetLineWidth(2);
  signal->Draw("HIST");
  random_background->Draw("HIST SAME");
  TLegend legend(0.38, 0.72, 0.84, 0.90);
  legend.SetBorderSize(0);
  legend.SetFillStyle(0);
  legend.AddEntry(signal, "Fast-coincidence window", "l");
  legend.AddEntry(random_background, "Random window", "l");
  legend.Draw();
  TLatex label_a;
  label_a.SetNDC();
  ...
  gPad->SetBottomMargin(0.15);
  gPad->SetTopMargin(0.04);
  style_axis_fast(*difference);
  difference->GetXaxis()->SetRangeUser(0.0, 100.0);
  difference->GetYaxis()->SetRangeUser(1e-1, 3e7);
