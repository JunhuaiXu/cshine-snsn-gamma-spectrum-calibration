#!/usr/bin/env python3
"""Redraw the two beam-off topology panels corresponding to note Fig. 18.

The final-selection beam-off output contains two ROOT histograms:

* ``ALL_h2_ax_ay_100_inf``: delta_y versus delta_x for E_tot > 100 MeV;
* ``ALL_h2_TotalE_CenterE``: E_core versus E_tot for the beam-off sample.

The producer uses the same central/side-core and plastic-veto selection as the
analysis-note Fig. 10 producer.  This program does not reconstruct candidates,
change that selection, rebin, normalize, smooth, or fit either histogram.  It
reads the two frozen objects and places them in a horizontal figure with
independent logarithmic count scales.
"""
...
DEFAULT_INPUT = Path(
    "DataPreprocessing/step7-DeltaYrelated/h2_check_BKG.root"
)
SPATIAL_OBJECT = "ALL_h2_ax_ay_100_inf"
ENERGY_OBJECT = "ALL_h2_TotalE_CenterE"
OUTPUT_STEM = "cshine_gamma_cosmic_muon_topology_horizontal"
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Draw the beam-off delta_y--delta_x and E_core--E_tot "
            "distributions produced with the final central/side selection."
        )
    )
    parser.add_argument(
        "--analysis-root",
...
def require_historical_binning(histogram, object_name):
    expected = EXPECTED_BINNING[object_name]
    ...
    if not matches:
        raise ValueError(
            "%s does not match the frozen final-selection binning: %d x %d, "
            "x=[%g, %g], y=[%g, %g]."
            % (
                object_name,
                expected["x_bins"],
...
def histogram_summary(histogram, counts):
    ...
    return {
        ...
        "outside_displayed_range": float(all_bins - in_range),
...
def read_source_histograms(input_path):
    ...
    try:
        result = []
        for object_name in (SPATIAL_OBJECT, ENERGY_OBJECT):
...
def draw_horizontal_figure(histograms, output_pdf, output_png):
    ...
    axes[0].set_xlim(0.0, 7.0)
    axes[0].set_ylim(0.0, 7.0)
    axes[0].set_xlabel(r"$\delta_x$ (cm)")
    axes[0].set_ylabel(r"$\delta_y$ (cm)")

    axes[1].set_xlim(5.0, 200.0)
    axes[1].set_ylim(0.0, 80.0)
    axes[1].set_xlabel(r"$E_{\mathrm{tot}}$ (MeV)")
    axes[1].set_ylabel(r"$E_{\mathrm{core}}$ (MeV)")
...
def write_metadata(path, input_path, root_version, histograms, outputs):
    metadata = {
        ...
        "physics_contract": {
            "sample": "beam-off reconstructed candidates in the final-selection output",
            "panel_a_selection": "E_tot > 100 MeV",
            "panel_b_selection": "no additional energy selection",
            "variables": [
                "panel (a): delta_x [cm], delta_y [cm]",
                "panel (b): E_tot [MeV], E_core [MeV]",
            ],
            "core_groups": (
                "central units 5, 6, 9, 10 plus side units 4, 7, 8, "
                "11, 13, 14"
            ),
            "veto_behavior": (
                "central candidates have no plastic-veto requirement; "
                "side candidates require count_veto == 0, meaning that all "
                "three plastic-veto detectors have no signal"
            ),
            "binning": {
                SPATIAL_OBJECT: "70 x 70 bins over [0, 7] cm on both axes",
                ENERGY_OBJECT: (
                    "50 x 80 bins over E_tot=[5, 200] MeV and "
                    "E_core=[0, 80] MeV"
                ),
            },
            "normalization": "none",
            "fit_or_transform": "none after the historical ROOT objects are filled",
...
def main():
    ...
    output_directory = args.output_dir.expanduser().resolve() / "cosmic_muon_topology"
